...
public class PlanningOfDayController {
    ...
    private void initializelabel(String date) throws SQLException {
        ...
        String s = "select tache.nomTache, tache.heureDebut, tache.heureFin, tache.description , alerte.priorité from tache, alerte where alerte.idtache = tache.idtache   AND tache.idutilisateur = ? and tache.date = ? ORDER BY ordrePriorite ";
        ...
        ArrayList<String> priorité = new ArrayList<>();
        ...
        for (int i = 0; i < nomsTaches.size(); i++) {
            ...
            Label label1 = new Label(priorité.get(i));
            label1.setId("coucou");
